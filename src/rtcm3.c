...
static int decode_type1029(rtcm_t *rtcm)
{
    ...
    if (i+60<=rtcm->len*8) {
        ...
        rtktrace(2,"rtcm3 1029 length error: len=%d\n",rtcm->len);
        return -1;
    }
    if (i+nchar*8>rtcm->len*8) {
        rtktrace(2,"rtcm3 1029 length error: len=%d nchar=%d\n",rtcm->len,nchar);
        return -1;
    } 
...
static int decode_type1041(rtcm_t *rtcm)
{
    ...
    if (i+482-12<=rtcm->len*8) {
        ...
        rtktrace(2,"rtcm3 1041 length error: len=%d\n",rtcm->len);
        return -1;
    }
    rtktrace(4,"decode_type1041: prn=%d iode=%d toe=%.0f\n",prn,eph.iode,eph.toes);
    ...
    if (!(sat=satno(sys,prn))) {
        rtktrace(2,"rtcm3 1041 satellite number error: prn=%d\n",prn);
        return -1;
    }
    eph.sat=sat;
    eph.week=adjgpsweek(week);
                            ...
                            int *iod, double *udint, int *refd, int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    rtktrace(4,"decode_ssr1_head: time=%s sys=%d subtype=%d nsat=%d sync=%d iod=%d"
                            ...
                            int *iod, double *udint, int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    rtktrace(4,"decode_ssr2_head: time=%s sys=%d subtype=%d nsat=%d sync=%d iod=%d"
         " provid=%d solid=%d\n",tstr,sys,subtype,nsat,*sync,*iod,provid,solid);
                            ...
                            int *hsize)
{
    ...
    *udint=ssrudint[udi];
    ...
    time2str(rtcm->time,tstr,2);
    rtktrace(4,"decode_ssr7_head: time=%s sys=%d subtype=%d nsat=%d sync=%d iod=%d"
          " provid=%d solid=%d\n",tstr,sys,subtype,nsat,*sync,*iod,provid,solid);
...
#if 0 /* for debug */
        rtktrace(2,"sig pos: sys=%d code=%d ex=%d idx=%d\n",sys,code[i],ex[i],idx[i]);
#endif
    }
}
...
static int decode_type1230(rtcm_t *rtcm)
{
    ...
    if (i+20>=rtcm->len*8) {
        rtktrace(2,"rtcm3 1230: length error len=%d\n",rtcm->len);
        return -1;
    }
    staid=getbitu(rtcm->buff,i,12); i+=12;
...
static int decode_type4076(rtcm_t *rtcm)
{
    ...
    if (i+3+8>=rtcm->len*8) {
        rtktrace(2,"rtcm3 4076: length error len=%d\n",rtcm->len);
        return -1;
    }
