...
extern int gis_read(const char *file, gis_t *gis, int layer)
{
    ...
    rtktrace(3,"gis_read file=%s layer=%d\n",file,layer);
    ...
    if (!(fp=fopen(file,"rb"))) { /* shapefile */
        rtktrace(2,"shapefile open error: %s\n",file);
        return 0;
    }
    if (!(fp_idx=fopen(path,"rb"))) { /* index file */
        fclose(fp);
        rtktrace(2,"shapefile index open error: %s\n",path);
        return 0;
    }
    /* read header */
    if ((type1=read_shape_head(fp))<0||(type2=read_shape_head(fp_idx))<0||
        type1!=type2) {
        rtktrace(2,"shapefile header error: %s type=%d %d\n",file,type1,type2);
        fclose(fp);
        fclose(fp_idx);
        return 0;
    }
